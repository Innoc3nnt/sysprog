...
coro_bus_delete(struct coro_bus *bus)
{
	...
	for (int i = 0; i < bus->channel_count; i++) {
		if (bus->channels[i] != NULL) {
			// Освобождаем память вектора данных
			free(bus->channels[i]->data.data);
			free(bus->channels[i]);
		}
	}
	
	// Освобождаем массив каналов
	free(bus->channels);
	// Освобождаем структуру шины
	free(bus);
}
...
coro_bus_channel_open(struct coro_bus *bus, size_t size_limit)
{
	...
	// Ищем свободный слот или расширяем массив каналов
	int channel_id = -1;
	for (int i = 0; i < bus->channel_count; i++) {
		if (bus->channels[i] == NULL) {
			channel_id = i;
	...
	if (channel_id == -1) {
		// Нужно расширить массив каналов
...
coro_bus_channel_close(struct coro_bus *bus, int channel)
{
	...
	// Сначала помечаем канал как закрытый
	bus->channels[channel] = NULL;

	// Будим все корутины, ожидающие на этом канале
...
coro_bus_try_send(struct coro_bus *bus, int channel, unsigned data)
{
	...
	// Проверяем, есть ли место в канале
	...
	// Будим первую корутину, ожидающую данные
...
coro_bus_try_recv(struct coro_bus *bus, int channel, unsigned *data)
{
	...
	*data = data_vector_pop_first(&ch->data);
	...
	// Будим первую корутину, ожидающую возможность отправить
